...
original_dataset = read.csv("training-tweets-alexa.csv" , stringsAsFactors = F)
...
#Run to load the already trained classifier.
classifier = readRDS("alexaclassifier.rds")


y_pred = predict(classifier, newdata = as(dtm.test.TF.IDF,"matrix") )
cm = table(tweets.test[,2],y_pred)
#For showing the accuracy.
confusionMatrix(cm)
#for saving individual variables.

# saveRDS(classifier , "alexaclassifier.rds")
# saveRDS(tweets.train , "alexatrainingtweets.rds")
# saveRDS(tweets.test , "alexatestingtweets.rds")
# saveRDS(y_pred , "alexaprediction.rds")
...
Totalalexasentiments = c("Positve" = totalpositive , 
                        "Negative" = totalnegative , 
                        "Neutral" = totalneutral)

#saveRDS(Totalalexasentiments , file = "alexatotal.rds")
